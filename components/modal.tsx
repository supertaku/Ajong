...
export function Modal({ open, onClose, title, children, footer, size = "medium", fullScreen = false, variant = "default" }: { open: boolean; onClose: () => void; title: string; children: ReactNode; footer?: ReactNode; size?: "small" | "medium" | "large"; fullScreen?: boolean; variant?: "default" | "auth" | "filter" | "reviews" | "rules" }) {
  const titleId = useId();
  ...
  return createPortal(<div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={panelRef} tabIndex={-1} className={`modal-panel modal-${size} ${fullScreen ? "modal-fullscreen" : ""} ${variant === "auth" ? "modal-auth" : ""} ${variant === "filter" ? "modal-filter" : ""} ${variant === "reviews" ? "modal-reviews" : ""} ${variant === "rules" ? "modal-rules" : ""}`} role="dialog" aria-modal="true" aria-labelledby={titleId}>
